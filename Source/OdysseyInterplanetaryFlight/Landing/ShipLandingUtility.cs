    ...
    if (snapshot.buildings != null)
    {
        ...
        private static void RestoreBuildings(ShipSnapshot snapshot, Map map, IntVec3 targetCenter, ref Thing restoredAnchor)
        {
            ...
            for (int i = 0; i < snapshot.buildings.Count; i++)
            {
                ...
                try
                {
                    // Явно выставляем ротацию на Thing перед спавном,
                    // чтобы она не сбрасывалась в North при восстановлении из сейва.
                    if (thing is Building building)
                    {
                        var rotField = typeof(Thing).GetField("rotation",
                            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                        rotField?.SetValue(thing, entry.rotation);
                    }

                    GenSpawn.Spawn(thing, desiredCell, map, entry.rotation, WipeMode.Vanish);
